...
app.post('/api/auth/login', authLimiter, async (req, res) => {
  try {
    ...
    user.loginHistory.unshift({ timestamp: new Date(), device: req.headers['user-agent'] || 'Unknown' });
...
// ============================================================
app.post('/api/orders/:id/return-request', protect, authorize('customer'), async (req, res) => {
  try {
    const { reason } = req.body;
    const order = await Order.findById(req.params.id);
    if (!order) return res.status(404).json({ message: 'Order nahi mila' });
    if (order.customer.toString() !== req.user._id.toString()) return res.status(403).json({ message: 'Ye order aapka nahi hai' });
    if (order.status !== 'delivered') return res.status(400).json({ message: 'Sirf delivered order ka return kar sakte ho' });

    order.returnRequest = { status: 'requested', reason, requestedAt: new Date() };
    await order.save();
    res.json({ message: 'Return request bhej di gayi, admin review karega', order });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});

app.get('/api/admin/returns', protect, authorize('admin', 'member'), async (req, res) => {
  const orders = await Order.find({ 'returnRequest.status': 'requested' }).populate('customer', 'name phone');
  res.json(orders);
});

app.put('/api/orders/:id/return-status', protect, authorize('admin', 'member'), async (req, res) => {
  try {
    ...
    await order.save();
    await notifyUser(order.customer, `Aapka return request "${status}" ho gayi hai (Order ${order.orderNumber})`, 'order');
    res.json({ message: `Return ${status} kar diya`, order });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
// ============================================================
// STOCK UPDATE (supplier apna stock quickly update kar sake)
// ============================================================
app.put('/api/products/:id/stock', protect, authorize('supplier'), async (req, res) => {
  try {
    const { stock } = req.body;
    if (stock === undefined || stock < 0) return res.status(400).json({ message: 'Sahi stock number dein' });

    const product = await Product.findOne({ _id: req.params.id, supplier: req.user._id });
    if (!product) return res.status(404).json({ message: 'Product nahi mila ya aapka nahi hai' });

    product.stock = stock;
    await product.save();
    res.json({ message: 'Stock update ho gaya', product });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
// ANALYTICS ROUTES
// ============================================================
app.get('/api/admin/analytics', protect, authorize('admin', 'member'), async (req, res) => {
  try {
    ...
    const totalSuppliers = await User.countDocuments({ role: 'supplier', 'supplierDetails.isVerified': true });
    const totalCustomers = await User.countDocuments({ role: 'customer' });

    res.json({
      totalOrders, totalRevenue: totalRevenue[0]?.total || 0, statusCounts, topProducts,
...
app.get('/api/supplier/analytics', protect, authorize('supplier'), async (req, res) => {
  try {
    const orders = await Order.find({ 'items.supplier': req.user._id });
    let totalEarned = 0, totalItemsSold = 0;
    orders.forEach((order) => {
      order.items.filter((i) => i.supplier.toString() === req.user._id.toString()).forEach((i) => {
        totalEarned += i.supplierPayoutAmount;
        totalItemsSold += i.quantity;
      });
    });
    const totalProducts = await Product.countDocuments({ supplier: req.user._id });
    res.json({ totalOrders: orders.length, totalEarned, totalItemsSold, totalProducts });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
// ============================================================
app.get('/api/auth/login-history', protect, async (req, res) => {
